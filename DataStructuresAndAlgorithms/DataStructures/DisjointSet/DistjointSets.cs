...
{
    public class DistjointSets<T> : IEnumerable<T>
    {
        ...
        public DistjointSets(IEnumerable<T> collection)
        {
        ...
        public T PathCompression(T value)
        {
            if (value.Equals(set[value].Value))
                return value;

            return set[value].Value = FindSet(value);
        }
